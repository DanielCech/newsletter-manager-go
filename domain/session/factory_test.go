...
func Test_Factory_ParseAccessToken(t *testing.T) {
	...
	expectedClaims := Claims{
		AuthorID: id.Author(uuid.MustParse("f650e26c-bce0-48b2-9ea4-126193d341e0")),
		Custom:   CustomClaims{UserRole: user.RoleUser},
	}
...
func Test_Factory_NewRefreshToken(t *testing.T) {
	...
	refreshToken, err := factory.NewRefreshToken(authorID)
	assert.NoError(t, err)
	assert.Equal(t, timeSource, refreshToken.timeSource)
	assert.NotEmpty(t, refreshToken.ID)
	assert.Equal(t, authorID, refreshToken.AuthorID)
...
func Test_Factory_NewRefreshTokenFromFields(t *testing.T) {
	...
	authorID := id.Author(uuid.MustParse("4049b2e8-767e-475e-b78d-6baaf356cdc1"))
	expiresAt := now.Add(refreshTokenExpiration)
	createdAt := now

	refreshToken := factory.NewRefreshTokenFromFields(refreshTokenID, authorID, expiresAt, createdAt)
	assert.Equal(t, timeSource, refreshToken.timeSource)
...
func Test_signAccessToken(t *testing.T) {
	...
	require.NoError(t, err)
	claims := Claims{
		AuthorID: id.Author(uuid.MustParse("f650e26c-bce0-48b2-9ea4-126193d341e1")),
		Custom:   CustomClaims{UserRole: user.RoleUser},
	}

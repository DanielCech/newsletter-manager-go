...
import (
	...
	gqlapi "strv-template-backend-go-api/api/graphql/graph"
...
func setupController(database sql.Database, logger *zap.Logger, cfg config) (*gqlapi.Controller, error) {
	...
	*sessionService = *sessionServiceTmp
	...
	controller, err := gqlapi.NewController(
		userService,
		sessionService,
		sessionFactory,
		database,
	...
	if err != nil {
		return nil, fmt.Errorf("new graphql controller: %w", err)
	}
